...
function SkillCard({ skill }: SkillCardProps) {
  ...
  return (
    <div className="bg-gray-800 p-3 sm:p-4 rounded-lg hover:bg-gray-700 transition-colors duration-300 group">
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        <h4 className="text-sm sm:text-base font-semibold text-white group-hover:text-teal-400 transition-colors">
          {skill.name}
        </h4>
        <span className={`px-2 py-1 text-xs font-medium rounded-full text-white ${getSkillLevelColor(skill.level)}`}>
          <span className="hidden sm:inline">{getSkillLevelText(skill.level)}</span>
          <span className="sm:hidden">{skill.level}/5</span>
        </span>
      </div>
      ...
      <div className="w-full bg-gray-600 rounded-full h-1.5 sm:h-2">
        <div
          className={`h-1.5 sm:h-2 rounded-full transition-all duration-500 ${getSkillLevelColor(skill.level)}`}
...
function SkillsSection() {
  ...
  return (
    <section id="skills" className="py-12 sm:py-16 md:py-20 px-4 sm:px-8 bg-gray-800/50">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12 sm:mb-16">
          <h3 className="text-3xl sm:text-4xl font-bold text-white mb-4">Skills & Technologies</h3>
          <p className="text-base sm:text-lg text-gray-300 max-w-2xl mx-auto mb-2 px-4 sm:px-0">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8 sm:mb-12">
          ...
          {SkillCategories.map((category) => (
            <button
              ...
              className={`px-4 sm:px-6 py-2 sm:py-3 rounded-lg text-sm sm:text-base font-medium transition-all duration-300 flex items-center gap-1 sm:gap-2 ${
          ...
          {filteredSkills.length > 8 && !isExpanded && (
            <div className="text-center mt-8 sm:mt-10">
              <button
                ...
                className="px-6 sm:px-8 py-3 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-full transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2 mx-auto text-sm sm:text-base"
              >
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
                <span className="hidden sm:inline">Show All Skills & Categories</span>
          ...
          {isExpanded && activeCategory !== 'all' && filteredSkills.length > 8 && (
            <div className="text-center mt-8 sm:mt-10">
              <button
                ...
                className="px-6 sm:px-8 py-3 bg-gray-700 text-teal-400 border border-teal-400 hover:bg-gray-600 font-semibold rounded-full transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2 mx-auto text-sm sm:text-base"
              >
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
